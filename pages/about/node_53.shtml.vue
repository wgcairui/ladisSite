    ...
    <b-row no-gutters>
      ...
      <b-col cols="12" md="9" class="p-5">
        <b-container>
          <b-row no-gutters>
            <b-col cols="12">
              <h4 class="text-center m-2">经销商列表</h4>
              <hr />
            </b-col>
          </b-row>
          <b-row no-gutters>
...
<script lang="ts">
  ...
  export default Vue.extend({
    async asyncData({ app, $Api, error }) {
      ...
      const area = {}
      // dealear经销商信息
      const dealers = await $Api.getBuyList().then(el => el.filter(els => els.MainParent === '经销商列表'))
      // 构建销售服务中心列表
      ...
      if (!area) { return error({ statusCode: 500, message: '页面走丢了' }) }
      return { map, area }
    },
