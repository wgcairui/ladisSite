        ...
        <b-tabs content-class="mt-3" justified>
          <b-tab v-for="val in downs" :key="val.title" :title="val.title">
            <b-list-group>
              ...
              <b-list-group-item v-for="v1 in fileTypeFilter(val.data, 'soft')" :key="v1.title">
                <b-button v-b-toggle="`coll${encodeURI(v1.title)}`" variant="link">{{ v1.title }}</b-button>
                <b-collapse
...
<script lang="ts">
  import Vue from 'vue'
  import { supportAsid } from '~/types/typing'
  export default Vue.extend({
    async asyncData({ $Api, params }) {
      const title = params.id
      const supportAsid = await $Api.getPagesType<supportAsid>("support_asid")
      const downs = await $Api.getSupport(title)
