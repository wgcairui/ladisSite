...
export default Vue.extend({
  ...
  async asyncData ({ app }) {
    const listArray = await app.$Api.GeneralGetInfo({ table: 'VR' }).then((el:vrList[]) => {
      return el.map(e => e.data).flat()
    })
    return { listArray }
  },
  data () {
  ...
  methods: {
    vrFilter (type: string = '') {
      if (!type) {
        this.backListArray = this.$data.listArray
      } else {
        const list: any[] = []
        switch (type) {
          case 'dev':
            this.$data.listArray.forEach((el: { name: string }) => {
              if (el.name.includes('产品全景展示')) {
                list.push(el)
              }
            })
            break
          case 'house':
            this.$data.listArray.forEach((el: { name: string }) => {
              if (el.name.includes('机房全景展示')) {
                list.push(el)
              }
            })
            break
        }
        this.$data.backListArray = list
      }
    }
    /* Serize (href: string) {
