#!/usr/bin/env bash
# ==============================================================================
# ladis-site 裸机部署单文件脚本
#
# 适用场景：目标机器没装 Docker / 不想用 Docker / 临时部署
# 行为：
#   - 从 .env 加载环境变量（不存在则提示创建）
#   - 检测 Node.js（要求 22+），自动 npm ci + nuxt build
#   - 用 setsid + nohup 把 nuxt start 放到后台跑（不依赖 pm2 / systemd）
#   - PID 写入 .run/ladis-site.pid，日志写入 logs/ladis-site.log
#   - 端口默认 80（生产），可用 PORT env 覆盖
#
# 使用：
...
PID_DIR=".run"
PID_FILE="$PID_DIR/ladis-site.pid"
LOG_DIR="logs"
LOG_FILE="$LOG_DIR/ladis-site.log"
ENV_FILE=".env"
...
cmd_uninstall() {
  cmd_stop || true
  log "删除 node_modules / .nuxt / .run / logs ..."
  rm -rf node_modules .nuxt "$PID_DIR" "$LOG_DIR"
  ok "已清理"
}
